...
# from analysis.constant import (DTYPE_CONFIG_DIR_NAME, GPU_CONFIG_DIR_NAME,
...
class ModelConfig:
    ...
    # 分组卷积的时候，需要指导K和V的head数目，llama2-70b和llama3中都需要使用
    ...
    mlp_gated_linear_units: bool = False  # whether to use gated linear units for MLP，silu函数的门控
    # 后初始化一些参数
...
class ParallelismConfig:
    ...
    # deepspeed的数据并行实现
    dp_size: int = (
        1  # sharded data parallelism size, PyTorch FSDP or DeepSpeed Zero parallelism implementation  # 这个是采用切分策略的数据并行，参考deepspeed
    )
    # 普通的数据并行实现
    rdp_size: int = 1  # replicated data parallelism size, PyTorch HSDP implementation    # 这个是常见的数据并行
    ep_size: int = 1  # expert parallelism size   专家并行
...
def dump_configs(configs: dict, config_dir_name: str) -> None:
    ...
    for k, v in configs.items():
        with open(Path(__file__).parent / Path(config_dir_name, f"{k}.json"),"w") as f:
            json.dump(v, f, cls=EnhancedJSONEncoder, indent=4)  # 注意这里，将config转换为可以保存的类型
    logger.info(f"dumped {len(configs)} configs to {config_dir_name}")
    

# 直接从huggingface获取模型配置
def get_model_config_from_hf(name: str, ) -> ModelConfig:
    ...
    if AutoConfig is None:
        logger.warning(f"cannot import AutoConfig from transformers, `transformers` is not installed, HuggingFace will not be available to use for model config retrieval")
        return None
    ...
    if hasattr(hf_config, "moe_num_experts"):
        ...
        moe_num_experts = 1
        logger.info("hf config does not have moe_num_experts or num_local_experts, setting moe_num_experts = 1 (not MoE model)")
    ...
    if ffn_embed_dim:
        ...
        if expansion_ratio == 3.5:
            mlp_gated_linear_units = True      # 如果等于3.5的话，则使用了门控激活
    else:
...
def get_hf_models_by_type_and_task(
    ...
    try:
        ...
        logger.error(f"cannot import HfApi from huggingface_hub, lease install huggingface_hub first")
    api = HfApi()
...
# 从本地读取模型、集群和数据类型的参数配置，作为全局变量。【全局变量】
def populate_model_and_gpu_configs() -> None:
    ...
    #                              type="dtype")
    model_configs = read_configs(os.path.join(str(Path(__file__).parent), MODEL_CONFIG_DIR_NAME), type="model")
    gpu_configs = read_configs(os.path.join(str(Path(__file__).parent), GPU_CONFIG_DIR_NAME), type="gpu")
    dtype_configs = read_configs(os.path.join(str(Path(__file__).parent), DTYPE_CONFIG_DIR_NAME), type="dtype")
    logger.info(f"Populated {len(model_configs)} model configs, {len(gpu_configs)} gpu configs, {len(dtype_configs)} dtype configs")
...
# 根据模型名称获取模型配置，从configs，再到文件，再到huggingface上【如果不用huggingface，这块可以删掉】
def get_model_config_by_name(name_or_path: str) -> ModelConfig:
    ...
    if model_config is None:
        raise ValueError(f"unknown model config name: {name_or_path}, and none is found on HuggingFace Hub")
    return model_config
...
def get_gpu_config_by_name(name: str) -> GPUConfig:
    ...
    # print(gpu_configs, "-----test print gpu-configs") # 如果需要加入新的设备，是需要重新编译一下的， pip install .
...
# 将根据名称获得的模型配置信息打包成json保存起来
def dump_model_config_by_name(name: str, 
                              config_dir_name: str = MODEL_CONFIG_DIR_NAME) -> None:
...
# 根据任务下载topk的模型配置，然后将这些配置保存到config文件夹下
...
if __name__ == "__main__":
    # print(Path(__file__), __file__, Path(__file__).parent)
    ...
    fire.Fire(
        ...
        serialize=lambda x: json.dumps(x, cls=EnhancedJSONEncoder, indent=4) if dataclasses.is_dataclass(x) else x,
    )
